...
const NAV: NavItem[] = [
  ...
  { href: "/admin/entities", label: "Admin", icon: ShieldCheck, tool: "admin" },
];

export function Sidebar({ access }: { access: CurrentAccess }) {
  ...
  return (
    <aside className="hidden w-60 shrink-0 flex-col border-r border-border bg-surface md:flex">
      ...
      <nav className="flex-1 space-y-1 px-3 py-4">
        {NAV.filter((item) => item.tool === "always" || access.permissions[item.tool]?.view).map((item) => {
